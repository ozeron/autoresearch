...
export function registerRunTool(
  server: McpServer,
  setLastRunChecks: (checks: LastRunChecks | null) => void,
  getProjectDir: () => string
): void {
  server.tool(
    "run_experiment",
    "Run a benchmark or experiment command, optionally validate with checks script, and report results.",
    {
    ...
    async (params) => {
      ...
      if (benchmarkPassed && checksExists) {
        const checks = await spawnCommand(
        ...
        responseLines.push("");
        responseLines.push(
          `Checks: ${checksStatus} (${checks.durationSeconds.toFixed(1)}s)`
        );

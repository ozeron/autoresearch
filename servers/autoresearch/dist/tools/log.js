 ...
 */
export function registerLogTool(server, getState, setState, getLastRunChecks, clearLastRunChecks, incrementExperiments, getProjectDir) {
    server.tool("log_experiment", "Log a completed experiment run. On 'keep' status, auto-commits the working tree. Records metric, secondary metrics, status, and description.", {
        ...
        if (params.status === "keep") {
            ...
            try {
                child_process.execSync(`git add -A && git commit -m "${commitMsg.replace(/"/g, '\\"')}"`, { cwd: projectDir });
        ...
        fs.writeFileSync(path.join(projectDir, ".autoresearch-active"), String(Date.now()));

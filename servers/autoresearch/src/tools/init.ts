...
export function registerInitTool(
  ...
  server.tool(
    "init_experiment",
    "Initialize or reinitialize an experiment. Sets the primary metric name, unit, and direction (lower/higher is better). On reinitialization, previous results are preserved in a separate segment.",
    {
      ...
      metric_unit: z
        ...
        .optional()
        .describe(
          "Unit: 'µs', 'ms', 's', 'kb', 'mb', or '' for unitless. Default: auto-detect or ''"
        ),
      direction: z
        .string()
        .optional()
    ...
    async (params) => {
      ...
      const resolvedUnit =
        metric_unit !== undefined ? metric_unit : detectMetricUnit(metric_name);
      ...
      const resolvedDirection: "lower" | "higher" = rawDirection;
